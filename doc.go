...
second or couple of seconds by the receiver on a serial or USB port.
For a given receiver, the sentences are usually repeated identically in
each cycle, at least as far as their types are concerned. For instance:
...
GPGSV sentence per NMEA cycle is not handled very gracefully. In this
situation, the satellites are cleared when the first GPGSV sentence of
a burst is encoutered and only the cycle containing the last GPGSV
sentence will give a fix with the actual number of satellites.
